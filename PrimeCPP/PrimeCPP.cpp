...
void runFor(double s) {
    ...
    while (true) {
        ...
        sieve.runSieve();
        passes++;
        if (duration_cast<seconds>(steady_clock::now() - tStart).count() >= s)
        {
...
void runOnce(long count) {
    auto tStart = steady_clock::now();
    prime_sieve sieve(count);
    sieve.runSieve();
    printf("%d ms\n", duration_cast<milliseconds>(steady_clock::now() - tStart).count());
}