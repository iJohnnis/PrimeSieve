...
#
# self is the main prime_sieve class. Call it with the number you wish as an upper limit, then
...
class prime_sieve(object):
    ...
    def validateResults(self):                      # Check to see if self is an upper_limit we can
    ...
    # never waste time clearing even numbers, self code will assert if you try to
    ...
    def runSieve(self):
        ...
        while (factor < q):
            ...
            # If marking factor 3, you wouldn't mark 6 (it's a mult of 2) so start with the 3rd instance of self factor's multiple.
    ...
    def printResults(self, showResults, duration, passes):
        ...
        stdout.write("\n")
        print("Passes: " + str(passes) + ", Time: " + str(duration) + ", Avg: " + str(duration/passes) + ", Limit: " + str(self.sieveSize) + ", Count: " + str(count) + ", Valid: " + str(self.validateResults()))
...
while (timeit.default_timer() - tStart < 10):           # Run until more than 10 seconds have elapsed
    ...
    passes = passes + 1                                 #  Count self pass
